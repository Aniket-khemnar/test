...
public class RestaurantDao implements DaoInterface<Restaurant,Integer> {
	...
	public Collection<Restaurant> getAll() {
		String sqlQuery = "select rest_id, rest_name, rest_cuisine, rest_branches from restaurant";
	...
	public Restaurant getOne(Integer id) {
		...
		try(
			...
			String sqlQuery = 
					"select rest_id, rest_name, rest_cuisine, rest_branches from restaurant where rest_id = ?";
	...
	public void create(Restaurant newRestaurant) {
		String sqlQuery =
		"insert into restaurant(rest_id, rest_name,rest_cuisine,rest_branches) values(?,?,?,?)";
		try(
				Connection conn = jdbcUtils.buildConnection();
				PreparedStatement pstmt = conn.prepareStatement(sqlQuery);
				){
	...
	public void update(Restaurant modifiedRestaurant) {
		String sqlQuery = "update restaurant set rest_name = ?, rest_cuisine = ?, rest_branches = ? where rest_id = ?";
		try(
				Connection conn = jdbcUtils.buildConnection();
				PreparedStatement pstmt = conn.prepareStatement(sqlQuery);
				){
